...
def create_custom_rabbitmq_recommendation_message_id_table():
  create_table_query = """
  CREATE TABLE IF NOT EXISTS custom_rabbitmq_recommendation_message_id (
    id SERIAL PRIMARY KEY,
    rabbitmq_message_id VARCHAR(255) NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
  )
  """
  try:
    print("Creating rabbitmq recommendation message id table...")
    connection = psycopg2.connect(**db_params)
    cursor = connection.cursor()
    cursor.execute(create_table_query)
    connection.commit()
...
def get_custom_rabbitmq_recommendation_message_ids():
  select_query = """
  SELECT rabbitmq_message_id FROM custom_rabbitmq_recommendation_message_id
  """
  try:
    connection = psycopg2.connect(**db_params)
    cursor = connection.cursor()
...
def update_custom_rabbitmq_recommendation_message_ids(rabbitmq_message_id, created_at):
  insert_query = """
  INSERT INTO custom_rabbitmq_recommendation_message_id (rabbitmq_message_id, created_at) VALUES (%s, %s)
  """
  try:
    connection = psycopg2.connect(**db_params)
    cursor = connection.cursor()
