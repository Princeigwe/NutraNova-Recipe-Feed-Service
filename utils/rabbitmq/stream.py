...
from utils.cursor_rabbitmq_postgres_operations import update_custom_rabbitmq_user_message_ids, update_custom_rabbitmq_recommendation_message_ids
import json
...
def stream_message(message):
  ...
  if message['type'] == chef_data_update_message_type:
    consumed_rabbitmq_user_message_ids = settings.RABBITMQ_USER_MESSAGE_IDS
    if message['message_id'] not in consumed_rabbitmq_user_message_ids:
    ...
    consumed_rabbitmq_recommendation_message_ids = settings.RABBITMQ_RECOMMENDATION_MESSAGE_IDS
    if message['message_id'] not in consumed_rabbitmq_recommendation_message_ids:
      print("Consuming user recommended feed rabbitmq message...")
      consume_user_recommended_feed(message)
      update_custom_rabbitmq_recommendation_message_ids(message['message_id'], message['created_at'])
    else: 
      print("Message already consumed")
    # consume_user_recommended_feed(message)
